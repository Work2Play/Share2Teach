...
const ModerationPage = () => {
  ...
  const csvData = filteredAndSortedData([...unverifiedPDFs, ...reportedPDFs]).map(pdf => ({
    ...
    subject: pdf.subject,
    userID: pdf.userID,
    modifiedAt: pdf.modifiedAt ? new Date(pdf.modifiedAt).toLocaleString() : 'N/A', // Ensure `modifiedAt` is converted to a date
          ...
          <tbody>
            {filteredAndSortedData(unverifiedPDFs).map((pdf) => (
              <tr key={pdf.id}>
                ...
                <td>{pdf.subject}</td>
                <td>{pdf.userID}</td>
                <td>{pdf.modifiedAt ? new Date(pdf.modifiedAt).toLocaleString() : 'N/A'}</td>
                <td>
